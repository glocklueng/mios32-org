...
#define STEREOSAMPLES 1			// 0: mono samples, 1: stereo samples
...
#if STEREOSAMPLES
	#define MAX_TIME_IN_DMA 25 		// Time (*0.1ms) to read sample data for, e.g. 40 = 4.0 mS
	#define SAMPLE_BUFFER_SIZE 256 // -> 512 L/R samples, 80 Hz refill rate (11.6~ mS period). DMA refill routine called every 5.8mS.
	#define POLYPHONY 7				// Max voices to sound simultaneously
...
static u8 sample_retrigger[NUM_SAMPLES_TO_OPEN]; //retrigger indicator
static u16 sample_decay[NUM_SAMPLES_TO_OPEN];	// Sample decay parameter (used to calculate per sample decrement value)
static u16 sample_decval[NUM_SAMPLES_TO_OPEN];	// Decay time in sample num read in from bank file
static u16 sample_decval1[NUM_SAMPLES_TO_OPEN];	// Decay time compensation factor
...
static u8 samplebyte_buf[POLYPHONY][SAMPLE_BUFFER_SIZE*(1+STEREOSAMPLES)];	// Create a buffer for each voice
...
void Open_Bank(u8 b_num)	// Open the bank number passed and parse the bank information, load samples, set midi notes, number of samples and cache cluster positions
{
  ...
  no_samples_loaded=0;
  no_decay=0; //1;						// Default to no decay for bank
   ...
   else
  {
		  for(samp_no=0;samp_no<NUM_SAMPLES_TO_OPEN;samp_no++)	// Check for up to the defined maximum of sample mappings (one per line)
		  {
			if(FILE_ReadLine(f_line, 63)) // Read line up to 63 chars long
			{
			   ...
			   sample_decay[samp_no]=127;
			   if(sample_decval[samp_no]>0) {
				   no_decay=0; 	// At least one of the samples requires decay processing
			   	   sample_decval[samp_no]=sample_decval[samp_no]*44; // Convert ms to ~sample num
			   	   if(sample_decval[samp_no]>32767) { sample_decval[samp_no]=32767; }	// Saturate
			   	   sample_decay[samp_no]=32767/sample_decval[samp_no]; // Calculate per sample decay
			   	   sample_decval1[samp_no]=32767%sample_decval[samp_no]; // Calculate compensation value
			   	   sample_decval[samp_no]=sample_decval[samp_no]-sample_decval1[samp_no]; // Compensate
					}
			   ...
			   DEBUG_MSG("Sample no %d, filename is: %s, midi note value=0x%x, decay value %d, 1 %d, y %d, hold=%d",samp_no,sample_filenames[samp_no],sample_to_midinote[samp_no],sample_decval[samp_no],sample_decval1[samp_no],sample_decay[samp_no],hold_sample[samp_no]);
			   sample_retrigger[samp_no]=0;
		 ...
		 for(samp_no=0;samp_no<no_samples_loaded;samp_no++)	// Open all sample files and mark all samples as off
		 {
		   ...
		   DEBUG_MSG("Open sample file failed.");
			 ...
			 for(cluster_ix=0; cluster_ix < CLUSTER_CACHE_SIZE; ++cluster_ix) {
			   u32 pos = cluster_ix*num_sectors_per_cluster*SAMPLE_BUFFER_SIZE*(1+STEREOSAMPLES);
	...
	if( midi_package.event == NoteOn && midi_package.velocity > 0 )
	{
				for(samp_no=0;samp_no<no_samples_loaded;samp_no++)	// go through array looking for mapped notes
				{
				 if(midi_package.note==sample_to_midinote[samp_no])		// Midi note on matches a note mapped to this sample samp_no
					{
					 	if (sample_on[samp_no]==0){
					 		sample_on[samp_no]=-1; 	// trigger the sample
					 		sample_vel[samp_no]=velocity_curve[midi_package.velocity];
					 		// Mark it as want to play unless it's already on
					 	}
					 	else {
					 		if (sample_on[samp_no]==-2) {sample_on[samp_no]=32767 ;} // release decay to smooth the note end if decay is 0
					 		sample_retrigger[samp_no]=velocity_curve[midi_package.velocity]; //retrigger value holds the velocity
					 		//DEBUG_MSG("Retrigger sample %d on , midi note %x hex",sample_retrigger[samp_no],midi_package.note);
					 	}
		...
		for(samp_no=0;samp_no<no_samples_loaded;samp_no++)	// go through array looking for mapped notes
		{
			if(midi_package.note==sample_to_midinote[samp_no])		// Midi note on matches a note mapped to this sample samp_no
			{
				...
				if (!hold_sample[samp_no] && !damper_pedal)	// if not holding sample or damper pedal not pressed, turn the note off, otherwise do nothing
				{
					if( sample_on[samp_no]<0 )
					{
						sample_on[samp_no]=32767 ; //sample_decval[samp_no];							// Mark it as decaying with the appropriate time for this sample
					}
...
void SYNTH_ReloadSampleBuffer(u32 state)
{
  ...
  s16 OutWavs16L;	// 16 bit output to DAC
  s16 OutWavs16R;	// 16 bit output to DAC
  s32 OutWavs32;	// 32 bit accumulator to mix samples into
  s32 SampleCalc; // Sample decay temp
  ...
  MIOS32_BOARD_LED_Set(0x1, 0x1);	// Turn on LED at start of DMA routine
	...
	if(voice_no)	// if there's anything to play, read the samples and mix otherwise output silence
	{
		for(voice=0;voice<voice_no;voice++) 	// read up to SAMPLE_BUFFER_SIZE characters into buffer for each voice
		{
			...
			if(SAMP_FILE_read(samplebyte_buf[voice],SAMPLE_BUFFER_SIZE*(1+STEREOSAMPLES),voice_samples[voice])<0) // Read in the appropriate number of sectors for each sample thats on
			...
			samplefile_pos[voice_samples[voice]]+=SAMPLE_BUFFER_SIZE*(1+STEREOSAMPLES);	// Move along the file position by the read buffer size
		...
		for(i=0; i<(SAMPLE_BUFFER_SIZE*(1+STEREOSAMPLES)); i+=2*(1+STEREOSAMPLES)) // Fill half the sample buffer
			{	
				OutWavs32=0;	// zero the voice accumulator for this sample output
				for(voice=0;voice<voice_no;voice++)
				{
					if (sample_retrigger[voice_samples[voice]]>0){ // if retrigger, short decay
						sample_on[voice_samples[voice]]-=128; // retrigger decaying
						if (sample_on[voice_samples[voice]]<0) {sample_on[voice_samples[voice]]=0;}
						SampleCalc=((sample_on[voice_samples[voice]])*(s16)((samplebyte_buf[voice][i+1] << 8) + samplebyte_buf[voice][i]))>>15;
						}
					else if (sample_on[voice_samples[voice]]>sample_decval[voice_samples[voice]]){
						sample_on[voice_samples[voice]]-=(sample_decay[voice_samples[voice]]+1);  // decaying with compensation
						if (sample_on[voice_samples[voice]]<0) {sample_on[voice_samples[voice]]=0;}
						SampleCalc=((sample_on[voice_samples[voice]])*(s16)((samplebyte_buf[voice][i+1] << 8) + samplebyte_buf[voice][i]))>>15;
						}
					else if (sample_on[voice_samples[voice]]>0){
						sample_on[voice_samples[voice]]-=(sample_decay[voice_samples[voice]]); // decaying wo compensation
						if (sample_on[voice_samples[voice]]<0) {sample_on[voice_samples[voice]]=0;}
						SampleCalc=((sample_on[voice_samples[voice]])*(s16)((samplebyte_buf[voice][i+1] << 8) + samplebyte_buf[voice][i]))>>15;
						}
 					else if (sample_on[voice_samples[voice]]<0){
						SampleCalc=((samplebyte_buf[voice][i+1] << 8) + samplebyte_buf[voice][i]);
						}
					else SampleCalc=0;

					OutWavs32+=voice_velocity[voice]*(s16)(SampleCalc);		// mix
				}
				...
				OutWavs16R = (s16)OutWavs32;					// Required to make following bit shift work correctly including sign bit

#if STEREOSAMPLES
				OutWavs32=0;	// zero the voice accumulator for this sample output
				for(voice=0;voice<voice_no;voice++)
				{
					if (sample_on[voice_samples[voice]]>0){
						SampleCalc=((sample_on[voice_samples[voice]])*(s16)((samplebyte_buf[voice][i+1+2*STEREOSAMPLES] << 8) + samplebyte_buf[voice][i+2*STEREOSAMPLES]))>>15;
						}
					else if (sample_on[voice_samples[voice]]<0){
						SampleCalc=((samplebyte_buf[voice][i+1+2*STEREOSAMPLES] << 8) + samplebyte_buf[voice][i+2*STEREOSAMPLES]);
						}
					else SampleCalc=0;

					OutWavs32+=voice_velocity[voice]*(s16)(SampleCalc);		// else mix it in
				}
				OutWavs32 = (OutWavs32>>SAMPLE_SCALING);	// Round down the wave to prevent distortion, and factor in the velocity multiply
				if(OutWavs32>32767) { OutWavs32=32767; }	// Saturate positive
				if(OutWavs32<-32768) { OutWavs32=-32768; }	// Saturate negative
#endif
				OutWavs16L = (s16)OutWavs32;					// Required to make following bit shift work correctly including sign bit

#if DAC_FIX
				*buffer++ = (OutWavs16L << 16) | (-OutWavs16R & 0xffff);	// make up the 32 bit word for L and R and write into buffer with fix for PCM1725 DAC
#else
				*buffer++ = (OutWavs16L << 16) | (OutWavs16R & 0xffff);	// make up the 32 bit word for L and R and write into buffer
#endif
				}
	}
...
s32 SYNTH_Init(u32 mode)
{
	...
	RCC_PLLI2SConfig((uint32_t)271,(uint32_t)2); //I2S PLL dividers for 44100 values from stm32f407 datasheet!!!
	MIOS32_I2S_Init (0);
#endif
			...
			if(new_voice_no<POLYPHONY)	// As long as not already at max voices, otherwise don't trigger/play
				{
				 ...
				 else if (sample_retrigger[samp_no]>0 && sample_on[samp_no]==0){ //if retrigger, restart the note after the  short decay
					sample_vel[samp_no]=sample_retrigger[samp_no];
					sample_on[samp_no]=-1;
					sample_retrigger[samp_no]=0;
					}
				}
